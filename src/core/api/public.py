...
def _resolve_get_exchange_client():
    ...
    get_exchange_client = getattr(server_mod, "get_exchange_client", None)
    if get_exchange_client is None:
        raise RuntimeError(
            "Batch E1 public candles verification requires `core.server.get_exchange_client`."
        )
