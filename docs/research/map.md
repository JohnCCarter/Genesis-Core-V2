...
| `slippage-backtest-methodology.md`                       | topic   | active | How should slippage be modeled in V2 backtest, and what holds when no order book exists?                  | `scripts/analyze/cost_stress_sweep.py`, `src/core/backtest/position_tracker.py`, GitHub Issue #12 |
| `infrastructure-fitness-audit.md`                        | topic   | active | Which V2 infrastructure is safe to reuse for a clean-slate candidate lane, and what needs guards?        | `seed_manifest.json`, `AGENTS.md`, `scripts/audit/find_new_champion_candidate.py`                |
| `clean-slate-candidate-factory.md`                       | topic   | active | How should V2 build a clean-slate, mechanism-first candidate lane that reuses the platform but resets the candidate? | `docs/adr/0003-research-tooling-non-authoritative.md`, `seed_manifest.json`, `infrastructure-fitness-audit.md`        |
