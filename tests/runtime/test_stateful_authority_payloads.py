...
def test_runtime_seed_baseline_is_admitted_and_loaded_when_runtime_override_is_absent(
    ...
    assert cfg["strategy_family"] == (seed_payload.get("cfg", {}).get("strategy_family") or "legacy")
    assert (
...
def test_runtime_json_override_precedence_stays_ahead_of_seed(
    monkeypatch, tmp_path: Path
) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    seed_source = repo_root / "config" / "runtime.seed.json"
    ...
    seed_payload["cfg"]["thresholds"]["entry_conf_overall"] = 0.26

    runtime_payload = json.loads(json.dumps(base_payload))
    runtime_payload["version"] = 9
    runtime_payload["cfg"]["thresholds"]["entry_conf_overall"] = 0.61
    runtime_payload["cfg"]["gates"]["cooldown_bars"] = 5
    ...
    assert cfg["thresholds"]["entry_conf_overall"] == 0.61
    assert cfg["gates"]["cooldown_bars"] == 5
...
def test_verified_champion_subset_is_admitted_and_missing_symbols_still_fallback() -> None:
    ...
    assert cfg_1h.source.replace("\\", "/").endswith(
        "config/strategy/champions/tBTCUSD_1h.json"
    )
    assert cfg_3h.source.replace("\\", "/").endswith(
    ...
    assert fallback.source == "baseline:1h"
